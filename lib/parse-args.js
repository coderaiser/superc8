...
import * as checkCoverage from '#commands/check-coverage';
...
export function buildYargs(withCommands = false) {
    const yargs = Yargs([])
        ...
        .option('responsive', {
            alias: 'R',
            default: ['1', 'true'].includes(env.SUPERC8_RESPONSIVE),
            type: 'boolean',
            describe: 'Use responsive coverage formatter (compact terminal-aware output with per-file %, uncovered lines). Supports --skip-full.',
        })
