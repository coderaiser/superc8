...
class ReportInstance {
    ...
    async run(overrides = {}) {
        if (this.responsive)
            return this.runResponsive(overrides);
    ...
    async runResponsive(overrides) {
        const {write = stdout.write.bind(stdout)} = overrides;
    ...
    async _getMergedProcessCovMaybeAsync() {
        if (!this.mergeAsync)
            return this._getMergedProcessCov();

        return await this._getMergedProcessCovAsync();
    }
    ...
    async getCoverageMapFromAllCoverageFiles() {
        ...
        const v8ProcessCov = await this._getMergedProcessCovMaybeAsync();
